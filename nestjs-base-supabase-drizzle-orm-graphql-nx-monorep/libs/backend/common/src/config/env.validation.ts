...
export function validateEnvironment(config: Record<string, unknown>) {
  ...
  const validatedConfig = plainToInstance(
    EnvironmentVariables,
    {
      ...
      GRAPHQL_GRAPHIQL_ENABLED: parseBoolean(
        config.GRAPHQL_GRAPHIQL_ENABLED,
        config.NODE_ENV !== Environment.Production,
      ),
      GRAPHQL_INTROSPECTION_ENABLED: parseBoolean(
  ...
  if (errors.length > 0) {
    throw new Error(
      `${MESSAGES.ENVIRONMENT_VALIDATION_FAILED}: ${errors.toString()}`,
    );
  }
