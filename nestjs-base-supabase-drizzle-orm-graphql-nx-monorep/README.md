...
Production-oriented Nx backend monorepo for NestJS APIs using PostgreSQL,
Supabase client access, Drizzle ORM, JWT authentication, session-based refresh
tokens, RBAC, isolated e2e testing, REST, GraphQL, Docker, and CI checks.
...
- Storefront public register and login endpoints
- Admin login endpoint without public registration
...
- Starter user rules:
  - admin API owns admin-only create/list/read/update/delete endpoints
  - storefront API owns current customer profile read/update endpoints
...
```text
apps/
  storefront-api/      customer-facing ecommerce API composition root
  admin-api/           privileged admin API composition root
libs/backend/
  auth/                shared auth services plus storefront/admin API modules
  users/               shared user service plus storefront/admin API modules
  database/            Drizzle schema, DB service, Supabase service, test DB guard
  common/              config, app setup, guards, pipes, filters, logger
test/                  storefront HTTP e2e tests and test env defaults
...
6. Start the storefront API.

```bash
npm run start:dev
```

The storefront API defaults to port `3000`. Start the admin API on port `3001`
in a second terminal when you need it:

```bash
npm run start:admin
```

Default storefront local endpoints:
...
The admin API uses the same paths under `http://localhost:3001` when started
with `npm run start:admin`.
...
The root `Dockerfile` builds a production Node image for the storefront API.
Create a sibling admin image or parameterize the build target when admin
deployment needs a separate container.
