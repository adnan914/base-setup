...
import { EcommerceApiModule } from '../apps/ecommerce-api/src/ecommerce-api.module';
import { DatabaseService } from '../libs/backend/src/database';
import { configureApp } from '../libs/backend/src/configure-app';

describe('Admin API (e2e)', () => {
  let app: INestApplication;
  let ecommerceApp: INestApplication;
  ...
  beforeAll(async () => {
    ...
    const ecommerceModuleFixture: TestingModule =
      await Test.createTestingModule({
        imports: [EcommerceApiModule],
      }).compile();

    ecommerceApp = ecommerceModuleFixture.createNestApplication();
    configureApp(ecommerceApp);
    await ecommerceApp.init();
  });
  ...
  afterAll(async () => {
    await resetDatabase(databaseService);
    await ecommerceApp?.close();
    await app?.close();
  });

  it('registers and logs in an ecommerce user', async () => {
    const email = `test-${Date.now()}@example.com`;

    await request(ecommerceApp.getHttpServer())
  ...
  it('rejects customer login on the admin API', async () => {
  ...
  it('does not expose public registration', async () => {
    await request(app.getHttpServer())
      .post('/api/v1/auth/register')
      .send({
        email: `admin-register-${Date.now()}@example.com`,
        firstName: 'Admin',
  ...
  it('revokes a session when a rotated refresh token is reused', async () => {
    ...
    const rotatedTokens = await request(ecommerceApp.getHttpServer())
  ...
  async function registerUser(email: string) {
    const response = await request(ecommerceApp.getHttpServer())
      .post('/api/v1/auth/register')
      .send({
        email,
        firstName: 'Http',
...
describe('Ecommerce API surface (e2e)', () => {
  let app: INestApplication;
  ...
  it('does not expose admin user listing', async () => {
