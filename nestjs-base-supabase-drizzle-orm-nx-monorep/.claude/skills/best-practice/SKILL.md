...
description: >
  ...
  Drizzle ORM, JWT auth, and related tooling. Use this skill whenever writing,
  reviewing, or scaffolding any backend code — including modules, controllers, services,
...
- Every endpoint uses `@ApiEnvelopeResponse` (not `@ApiOkResponse`) — it generates correct Swagger for the `{ success, data, message }` envelope
- `@Messages(MESSAGES.<KEY>)` sets the envelope message read by `ResponseInterceptor` — always pair it with `@ApiEnvelopeResponse`
- Use constants from `MESSAGES` — never hardcode message strings inline
- Every exposed endpoint gets `@ApiTags`, `@ApiOperation`, `@ApiEnvelopeResponse`, and all relevant error response decorators
...
- [ ] All new endpoints documented in Swagger with error response decorators
